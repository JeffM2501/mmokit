...
//     <id value="$Id: OverlayManager.cs 1213 2008-02-05 20:22:53Z borrillis $"/>
// </file>
...
namespace Axiom.Overlays
{
    ...
    public sealed class OverlayManager : ResourceManager
    {
        ...
        public bool HasViewportChanged
        {
            get
            {
                return viewportDimensionsChanged;
            }
        }
                ...
                if ( line.Length > 0 && !line.StartsWith( "//" ) )
                {
                    ...
                    if ( ( overlay != null && !skipLine ) || isTemplate )
                    {
                        ...
                        else if ( parms[ 0 ] == "entity" )
                        {
                            ...
                            if ( parms.Length != 3 )
                            {
                                Trace.Write( string.Format( "Bad entity line: {0} in {1}, expected format - entity meshName(entityName)'", line, overlay.Name ) );
                            } // if parms...
                            else
                            {
                                ParseHelper.SkipToNextOpenBrace( script );
